...
export function BranchesClient({ branches }: BranchesClientProps) {
  ...
  const table = useReactTable({
    ...
    meta: {
        viewBranch: (branch: Branch) => setViewingBranch(branch),
    },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage your laundry business locations.</p>
        </div>
        <CreateBranch />
      </div>
      ...
      <Card className="border-0 shadow-card">
        <CardHeader className="pb-4">
          <CardTitle className="text-lg font-semibold text-slate-900">All Branches ({table.getFilteredRowModel().rows.length})</CardTitle>
        </CardHeader>
        <CardContent className="pt-0">
          <Table>
            <TableHeader>
      ...
      {viewingBranch && (
        <BranchDetailsModal branch={viewingBranch} onClose={() => setViewingBranch(null)} />
      )}
    </div>
  );
}
