  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        {/* Use an onClick handler that stops propagation */}
        <DropdownMenuItem onSelect={(e) => {
          ...
          <div className="flex items-center w-full">
            <EditOrderButton 
              ...
              iconOnly={false}
              size="sm"
              className="h-auto p-0 font-normal justify-start w-full"
            />
          </div>
        </DropdownMenuItem>
...
export function OrdersClient({
  ...
  const [selectedPaymentStatus, setSelectedPaymentStatus] = useState(payment || "");
  ...
  const columns: ColumnDef<Order>[] = [
    ...
    },
    {
      ...
      enableSorting: true,
      cell: ({ row }) => {
        const totalAmount = row.original.amountDue || row.original.amount || 0;
        ...
        //   pendingAmount = 0;
        // } else if (row.original.paymentStatus === "Partially Paid") {
  ...
  const handleFilter = () => {
    ...
    const currentStatus = selectedStatus || undefined;
    const currentPayment = selectedPaymentStatus || undefined;
    
    // Don't send "all" to the backend - convert to undefined
    const statusFilter = currentStatus === "all" ? undefined : currentStatus;
    const paymentFilter = currentPayment === "all" ? undefined : currentPayment;
