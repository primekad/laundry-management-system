...
import { getOrdersByStatus } from '@/lib/data/reports';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getOrdersByStatus(branchId, period as string);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Failed to fetch orders by status data:', error);
