...
import { getProfitTrend } from '@/lib/data/reports';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getProfitTrend(branchId, period as string);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Failed to fetch profit trend data:', error);
