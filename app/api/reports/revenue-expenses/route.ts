...
import { getMonthlyRevenueExpenses } from '@/lib/data/reports';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getMonthlyRevenueExpenses(branchId, period as string);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Failed to fetch revenue vs expenses data:', error);
