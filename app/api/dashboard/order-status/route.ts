...
import { getOrderStatusDistribution } from '@/lib/data/dashboard';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const orderStatusData = await getOrderStatusDistribution(branchId);
    return NextResponse.json(orderStatusData);
  } catch (error) {
    console.error('Error fetching order status distribution:', error);
    return NextResponse.json({ error: 'Failed to fetch order status distribution' }, { status: 500 });
  }
}
